...
//
// These tests use self-signed certs generated by `openssl req`, which
// defaults to the empty SM2 distid (OpenSSL 3.x convention). Since
// gm-crypto 0.3.5 the default SM2 distid policy is `Strict` (RFC 5280
// §6.3-style fail-closed), so these tests opt-in to
// `DistidPolicy::Permissive { fallback_distids: vec![""] }` via
// `TlsConfig::with_distid_policy` (added in gm-tls 0.2.3 / PR-2.4).
// These tests exercise the TLS 1.3 wire format (handshake framing,
// record layer, app-data), not PKI — permissive distid is intentional.
// ===========================================================================
...
async fn test_loopback_handshake() {
    ...
    // OpenSSL-issued certs use the empty distid; opt-in to
    // permissive so the loopback exercises wire-format only.
    ...
    let server_config = TlsConfig::from_bytes(cert.clone(), key.clone(), ca.clone())
        ...
        .with_distid_policy(openssl_distid_policy.clone());
...
async fn test_loopback_echo_large_data() {
    ...
    // OpenSSL-issued certs use the empty distid; opt-in to permissive.
    let openssl_distid_policy = DistidPolicy::Permissive {
        fallback_distids: vec!["".to_string()],
        audit_on_fallback: None,
    };
