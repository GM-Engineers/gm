...
> 版本：1.2 | 适用于：gm-tls v0.2.1+ / gm-tlcp v0.6.4+ / gm-crypto v0.3.0+ / gm-kms v0.2.1+
...
|协议|版本字节|说明|
|--------------|---------|----------|
| **TLCP** | `0x0101` | 国密 TLCP 传输层密码协议（GB/T 38636-2020）。参考实现：`gm-tlcp` crate（独立维护中）。|
| **TLS 1.3 + SM** | `0x0303` | 本项目 `gm-tls` 实现的协议——标准 TLS 1.3（RFC 8446） + SM 密码套件（SM2/SM3/SM4）。**不是 TLCP**。|
| **TLS 1.3** | `0x0303` | 标准 TLS 1.3，无 SM 算法。|

> **重要**：协议版本字节 0x0101 与 0x0303 是**不兼容的不同协议**。
> 本项目 REST/gRPC listener 走的是 TLS 1.3 + SM (`0x0303`)，**不**支持 TLCP (`0x0101`)。
> 若需 TLCP 客户端互通，请使用独立 `gm-tlcp` crate。
