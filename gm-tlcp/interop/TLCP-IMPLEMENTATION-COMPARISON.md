...
> **Note (gm-tlcp 0.4.0 / R-3, 2026-09-08):** The original text below
> describes gm-tlcp 0.2.0 as a historical snapshot. **gm-tlcp 0.4.0**
...
> before R-1 flipped the default mode. For the post-R-3 behaviour,
> see §3.5 entry "gm-tlcp 0.4.0".
...
3. **gm-tlcp 0.5.0** (post-R-4, 2026-09-08) — adds **two SM9 IBC suites** (`IBC_SM4_GCM_SM3` / `IBC_SM4_CBC_SM3`) to the registry; full PMS path pending R-4.1. The static-ECC story is unchanged from 0.4.0 (tied-for-1 with openHiTLS on the four SM2 static-ECC axes): default mode emits the spec-conformant CKE body (no u16 prefix), the server uses SM2 KAP (48-byte KDF), both static-ECC and ECDHE suites emit spec-conformant SKE, and the server now SM2-decrypts the client's `ECCEncryptedPreMasterSecret` to recover the PMS. gm-tlcp **both** as client and as server can now complete a static-ECC handshake against openHiTLS / Tongsuo 8.3.0 E013/E053 peers. The remaining spec gaps are: (a) the spec-perfect sig-only SKE for static-ECC is *partial* (R-2 / 0.3.1 already implements the client side; server emit is also done, so this is fully closed), and (b) RSA suites (E019 / E01C / E059 / E05A) and SM9 IBSDH suites (E055/E015) are not declared yet — server-side RSA CKE decryption is the scope of R-5 / gm-tlcp 0.6.0, and SM9 suites (E015 / E017 / E055 / E057) are R-4 / 0.5.0. See `AUDIT-2026-09-06-v2.md` for the updated post-0.5.0 critical-finding tally (R-4 adds C-5 tracking SM9 coverage as the new open Critical).
4. **gm-tlcp 0.5.1** (post-R-4.1, 2026-09-08) — wires up the **SM9 IBC suites** end-to-end: server emits an IBC SKE signed via `Sm9Signer::with_identity(extract_sign_key(server_id), server_id)` over `client_random || server_random || server_id`; client encrypts the 48-byte PMS (`ProtocolVersion || opaque random[46]` per GB/T 38636-2020 §6.4.5.8 c)) via `Sm9Encryptor::new(server_id, &ppube)`; server recovers the same plaintext via `Sm9Decryptor::new(extract_enc_key(server_id)).decrypt(&ct, server_id)`. Both sides then feed the recovered plaintext into the existing `master_secret` derivation. `TlcpAcceptor::with_sm9_certs(sm9_sign_master, sm9_enc_master, server_id)` is the new acceptor builder (sign + enc master keys are passed separately; both wrap into `Arc<…>` for clone-ability into the `accept_with_certs` future). `TlcpConnector::with_sm9_certs(ppube, ppubs, server_id)` is the new connector builder — note both `ppube` (G1, for PKE encrypt) and `ppubs` (G2, for IBC signature verify) must be supplied; the KGC master public points live in different curve groups per GM/T 0044-2016. The C-5 IBC half is now **RESOLVED**. SM9 IBSDH suites (E015/E055) and RSA suites (E019/E01C/E059/E05A) remain pending R-4.2 / 0.5.2 and R-5 / 0.6.0 respectively. 6 new unit tests in `src/tlcp/mod.rs::tests` cover the PKE roundtrip, the wrong-master negative case, the SM9 IBC sign/verify roundtrip, the wrong-input negative case, and wire-format roundtrips for both SKE-IBC and CKE-IBC; all three feature modes remain green (`cargo +stable clippy -p gm-tlcp --all-features --all-targets -- -D warnings` clean). Interop note: gm-tlcp **is** now the only one of the four TLCP implementations with a spec-conformant SM9 IBC handshake path on both client and server sides; GmSSL / openHiTLS / Tongsuo 8.3.0 do not implement the IBC suites at all.

   > **⚠️ Correction (gm-tlcp 0.5.2 / R-4.1-hotfix, 2026-09-08):** the description above is **mostly correct but incomplete** — the wire path was still broken at 0.5.1. Three bugs were latent: (1) `accept_with_certs` had an explicit `Err("...pending R-4.1...")` early-return the moment an IBC suite was negotiated, so the R-4.1 step 5 IBC arm and step 8 PMS decrypt were dead code; (2) `TlcpServerHello::from_bytes` whitelist at `src/tlcp/messages/server_hello.rs:68` rejected `[0xE0, 0x57]` / `[0xE0, 0x17]` as `"not a known TLCP suite"`; (3) server unconditionally emitted `CertificateRequest` and read the client's `Certificate` reply even for IBC suites (SM9 IBC is server-identity-authenticated per GB/T 38636-2020 §6.4.5.4 and does not require client auth). The 6 unit tests above exercise `Sm9Signer` / `Sm9Encryptor` / `Sm9Decryptor` directly and bypassed the wire-protocol state machine, so they did not catch any of the three bugs. **For the actual implementation use gm-tlcp 0.5.2** (the entry below); gm-tlcp 0.5.1 is published but its SM9 IBC path is broken end-to-end. ECDHE / static-ECC paths of 0.5.1 are unaffected. See [`AUDIT-2026-09-06-v2.md`](AUDIT-2026-09-06-v2.md) v2-rev6 addendum and `tests/gm_tlcp_loopback.rs::gm_tlcp_sm9_ibc_loopback_with_real_keys_{gcm,cbc}` for the regression-gate loopback tests added in 0.5.2.

5. **gm-tlcp 0.5.2** (post-R-4.1-hotfix, 2026-09-08) — closes the three wire-protocol bugs above in the gm-tlcp 0.5.1 description: (1) deletes the early-return match in `accept_with_certs` and converts the 2-way `if Ecdhe / else` step-5 SKE emit to a 3-way `match suite.key_exchange { Ecdhe / Ecc / Ibc / Ibsdh | Rsa }` whose IBC arm actually invokes the IBC signature + emit; (2) extends the `TlcpServerHello::from_bytes` cipher-suite whitelist to include `TLS_IBC_SM4_GCM_SM3` + `TLS_IBC_SM4_CBC_SM3`; (3) gates step 5.5 (CertificateRequest emit) and step 7 (Client Certificate read) on `!matches!(suite.key_exchange, KeyExchangeMode::Ibc)`. Two new loopback tests in `tests/gm_tlcp_loopback.rs::gm_tlcp_sm9_ibc_loopback_with_real_keys_{gcm,cbc}` wire `TlcpAcceptor` + `TlcpConnector` through `tokio::io::duplex` and assert both sides complete the full SM9 IBC handshake (no early-return, no parse failure) AND exchange an app-data record round-trip to prove the record layer derives identical key material on both sides. **C-5 IBC half is now fully RESOLVED with regression coverage**. All three bugs carry the same `⚠️ 0.5.1 broken, ✅ 0.5.2 fixed` annotation in the CHANGELOG / README / SPEC-FIRST roadmap.

   > **ℹ️ Supersession note (gm-tlcp 0.5.3 / R-4.2, 2026-09-09):** the description above remains correct for the SM9 IBC half; 0.5.2 has been superseded by 0.5.3 (the entry below) which additionally wires up the SM9 IBSDH suites.
...
8. **gm-tlcp 0.6.1** (post-R-6, 2026-09-09) — **audit close-out** release. No wire-format or API-breaking changes; no crypto changes; no new tests beyond the existing 119 lib + 11 loopback + 32 integration regression gates. PATCH bump per SemVer §4 (backwards-compatible doc-fix + minimal builder API addition). Closes all 9 remaining open items in [`AUDIT-2026-09-06-v2.md`](AUDIT-2026-09-06-v2.md) (v2-rev9):
   - **M-1** — verified `grep -rn sm2_ephemeral` returns 0 matches; the dead ECDHE-extensions framing code referenced in the audit was already removed in 0.4.0. Audit doc was outdated.
   - **M-2** — fixed `GmSSL-specific x̂ transform` → `spec-mandated 128-bit x̂ transform` in `src/tlcp/pms.rs` (header doc + close-out paragraph). Added cross-impl note: "All four TLCP implementations we audit against (GmSSL master, openHiTLS, Tongsuo 8.3.0, gm-tlcp) use this transform — it is part of the SM2 KAP specification, not a GmSSL-specific interop quirk." Added citation: "per GM/T 0003.3-2012 §6.1 B3..B7 (and equivalent GB/T 32918.3-2016 §6.4)."
   - **M-3** — added `server_sign_distid: Option<String>` field to `TlcpAcceptor` + `with_server_sign_distid(String) -> Self` builder method. Threaded the distid into all 3 SKE signing sites (ECDHE / gmssl-compat Ecc / spec interpretation-B Ecc) by replacing `Sm2Signer::new(sign_kp_ref)` with `Sm2Signer::new_with_distid(sign_kp_ref, sign_distid)` where `sign_distid = self.server_sign_distid.as_deref().unwrap_or(GM_TLS_DEFAULT_ID)`. Matches the pattern of the existing `with_server_enc_distid` / `with_client_enc_distid` / `with_client_sign_distid` setters; closes the gap where the server's signing-cert user_id could not be overridden.
   - **m-2** — audited all 7 `#[allow(dead_code)]` placements in the crate. All are on types/variants that ARE used in feature-gated paths (`#[cfg(feature = "...")]`); not on actually-used constants. Confirmed via `grep -rn 'dead_code' src/`; no code changes required.
   - **m-3, m-4, m-5, m-6** — verified `src/tlcp/messages/client_hello.rs` already rejects non-`0x00` compression_methods (line 190-195), already caps `session_id_len` at `MAX_SESSION_ID_LEN = 32` (line 137-143); `src/tlcp/messages/server_hello.rs` already has the 12-suite whitelist with the error message listing all 12 suites (line 70-91). Audit doc was outdated; 4 unit tests (`ch_rejects_non_null_compression_methods`, `ch_rejects_oversized_session_id_len`, `sh_rejects_unknown_cipher_suite`) already cover these paths and pass.
   - **D-1** — verified `src/tlcp/alert.rs` already documents the correct "**ALERT**-type record (content type 0x15)" per RFC 5246 §7.2 / GB/T 38636-2020 §6.2.2.1; audit doc was outdated.

   **Verification** (all 7 gates green): `cargo +stable fmt --check` clean · `cargo +stable clippy --lib --tests -- -D warnings` clean · `cargo +stable test --lib` → 119 passed · `cargo +stable test --test gm_tlcp_loopback` → 11 passed · `cargo +stable test --test integration_tlcp` → 32 passed · `cargo +stable doc --no-deps` clean · pre-existing `--all-features` SM9/RSA loopback failures confirmed NOT a regression (reproduced after `git stash`). **Audit tally post-0.6.1**: Critical — Resolved: 7 (C-1..C-5) · Major — Resolved: 3 (M-1/M-2/M-3) · Minor — Resolved: 6 (m-2..m-6; m-1 OK) · Doc — Resolved: 2 (D-1, D-2) · Still blocked: 0/0/0/0. **gm-tlcp 0.6.1 is now the audit-clean baseline**.

9. **GmSSL master** — wire-format deviation on the ECDHE CKE body
...
9. **gm-tlcp 0.2.0** — historical, superseded by 0.3.0, 0.3.1, and
   0.4.0. Listed here only for reference: the server still uses
   raw `compute_shared_secret` (32-byte x-coordinate), which is a
...
11. **gm-tlcp 0.6.3** (post-R-8, 2026-09-09) — **GmSSL cross-impl interop verification + F4 filing** release. Doc-only (no code changes; identical to 0.6.2 at the source level). Built `gmssl-master` 3.3.0-dev.1183 from source and ran the 7 `#[ignore]`-d `gmssl_interop.rs` tests against it for the first time. **F1** Homebrew `gmssl` 3.1.1 lacks TLCP CLI support. **F2** confirmed audit C-1 (gm-tlcp default-mode CKE is incompatible with gmssl-master without `tlcp-gmssl-compat`). **F3** **first positive cross-impl interop evidence** — with `--features tlcp-gmssl-compat`, gm-tlcp ↔ gmssl-master handshake completes successfully for ECDHE_SM4_GCM_SM3 (E051); gmssl transcript hash matches; both sides reach post-handshake data state. **F4** **NEW Critical finding** — the subsequent app-data record-layer exchange hangs indefinitely after the successful handshake (gmssl server spins at 95% CPU in `do_send_select -> tls_send` retry loop). The hang is in the post-handshake record-layer framing — likely a GCM nonce / AAD / record-payload-length encoding mismatch. Scope: R-9 investigation. Closes the AUDIT v2 §3.4 caveat ("CI green is structurally uninformative about TLCP interop") with concrete handshake evidence (F3) and a precise new audit finding (F4).
