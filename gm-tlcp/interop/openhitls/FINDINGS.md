...
This is also written to `run/bundled_sign_pub.hex` by `run/extract-pub.sh`
so the test harness can read it via
`awk 'NR==1 { sub(/^0x/, ""); print }'` (regenerated, not tracked in git).
...
The `run/client_certs/` directory in this folder holds both certs (PEM,
DER, and PKCS#8 PEM keys), generated by `gen_client_certs.sh` and
verified against the openHiTLS CA. These are test artifacts and are
**not** tracked in git — re-run `gen_client_certs.sh` after a fresh
clone to regenerate them.
...
| `run/extract-pub.sh`                    | extract SM2 uncompressed pub from cert (regenerates `bundled_sign_pub.hex` and `server_sign_pub.hex`) |
| `run/server_sign_pub.{txt,pem,hex}`     | sign pub from self-generated sign.crt (regenerable, not tracked)  |
| `run/client_certs/`                     | client cert pair signed by openHiTLS CA (regenerated by `gen_client_certs.sh`, not tracked) |
