...
- ✅ 125 lib tests + 21 loopback tests（6 个新增 ECDHE/ECC/RSA-SHA256 loopback 门禁，在 `tests/gm_tlcp_loopback.rs`；gm-tlcp 作 client+server 经 `tokio::io::duplex` 完成完整握手 + app-data 来回） + 32 integration tests (`tests/integration_tlcp.rs`) + 4 default gmssl interop tests (7 more `#[ignore]`d, run with `--ignored` when `gmssl` is on `PATH`)
...
- ✅ R-8 / gm-tlcp 0.6.3: GmSSL 交叉实现互操作验证（文档更新）。从源码编译 gmssl-master 3.3.0-dev.1183 首次跨实现互操作正证据（带 `--features tlcp-gmssl-compat`）：gm-tlcp ↔ gmssl master 9 个握手消息交换成功，gmssl transcript hash 匹配，server Finished 发送成功，client Finished 验证成功。同时发现 1 个 new Critical：F4=记录层 app-data 交换在成功握手后挂起（gmssl 95% CPU 忙等），scope = R-9。详细见 `CHANGELOG.md [0.6.3]` + `AUDIT-2026-09-06-v2.md` v2-rev11。
- ✅ **R-11 / gm-tlcp 0.6.4**（2026-09-11 发布）**凑齐 12 项进程内 loopback 门禁 + 修复 2 个隐含 wire-format bug**：新增 6 个 loopback 门禁覆盖 4 个 ECDHE/ECC 套件 (E051/E011/E053/E013) 与 2 个 RSA-SHA256 套件 (E05A/E01C) ； `tests/gm_tlcp_loopback.rs` 总数 13 → 21。**Bug 1**：`accept_with_certs` step 8 服务端 ECDHE PMS 计算中 `Z_client` 取错证书 — `client_certs.first()` (sign cert) 应改为 `client_certs.get(1)` (enc cert)；pre-fix 签名记录 key 与 key-agreement key 是不同 SM2 keypair ，导致 Z_client 多样不一致、master_secret 发散，体现在 GCM authentication failed / CBC invalid padding 三个错误 (ECDHE 每个握手都中招)。**Bug 2**：connector static-ECC 路径手工拼接 CKE body 总是带冗余 `uint16` 长度前缀（`[u16 prefix ‖ ciphertext]`），但 spec-default 模式解析器仅在 `--features tlcp-gmssl-compat` 下剥前 缀；导致 `Sm2Decryptor::decrypt` 总是看到错误起始字节，报错 "ciphertext must start with 0x534D/0x30/0x04"。修复方法：全程走 `TlcpClientKeyExchange::new_ecc(.).to_bytes()` 以跟随活跃 feature flag（与 ECDHE/RSA/IBC/IBSDH 分支保持一致）。详细见 `CHANGELOG.md [0.6.4]`。
- ✅ R-9 / F4 重新分类为 External-Upstream-Blocker（审计 v2-rev12 变动）。R-9.1 同时抓取 `sample(1)` 两进程挂起状态：gmssl server 99.6% 在 `do_send_select → __select`；gm-tlcp client 100% 在 `mio::Selector::select`；netstat server recv-Q=59/client recv-Q=0 — classic deadlock。R-8 3 个 record-layer 假设 (H1 nonce/H2 AAD/H3 CBC padding) 全部经源码对比排除。F4 定位于 gmssl-master `tools/tlcp_server.c::do_send_select` state-machine 缺陷；R-9 本身不发单独的补丁版本，被合并进 R-11 / 0.6.4 一同发布。详细见 `gm-tlcp/interop/F4-DIAGNOSTIC-2026-09-09.md` + `CHANGELOG.md [0.6.4]`。
- ✅ R-10 / 外部上游跟踪提交与验证（2026-09-10，审计 v2-rev13）。GmSSL 上游 issue [#1920](https://github.com/guanzhi/GmSSL/issues/1920) 已由 EricZHANG1688 提交（F4 deadlock，针对 `tools/tlcp_server.c::do_send_select`；body 走 `gh issue create` + `gh issue edit` 两步去除草稿元信息，最终 185 行干净 body）。Tongsuo #836 comment 经 GitHub REST API 核实 EricZHANG1688 已在 2026-09-06 手工提交（comment id 5557341448，239 行 bundled repro draft）；Tongsuo 维护者 pr000000f 2026-09-09 回复确认 NTLS 根因并提交 fix，开了 sub-issue [#840](https://github.com/Tongsuo-Project/Tongsuo/issues/840) (Path 2) + [#841](https://github.com/Tongsuo-Project/Tongsuo/issues/841)（两个目前都是空 placeholder）；gm-tlcp 不会主动填充这两个 issue body。R-10 本身无生产代码改动，被合并进 R-11 / 0.6.4 一同发布。详细见 `gm-tlcp/interop/R9-4C-SUBMISSION-RECORD.md` + `gm-tlcp/interop/R10-STATUS-RECORD.md` + `AUDIT-2026-09-06-v2.md` v2-rev13 + `CHANGELOG.md [0.6.4]`。
- ❌ Tongsuo 8.3.0 round-trip — tongsuo-side NTLS state-machine 拒绝 `0x0101`， 调查见 `interop/tongsuo/upstream/`（同上 R-10：#836 + #840 + #841 已在上游受理）
